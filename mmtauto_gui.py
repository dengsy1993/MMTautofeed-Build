            ...
            if self.mode in ['publish', 'auto']:
                    ...
                    try:
                        ...
                        for retry in range(3):
                            try:
                                ...
                                else: self.emit_log(f"图床拒绝: HTTP {img_res.status_code}", "ERROR")
                                break 
                            except requests.exceptions.Timeout:
                                self.emit_log(f"⚠️ 图床超时，发起重试 {retry+1}/3", "WARNING")
                                if retry == 2: raise
                    except Exception as e: self.emit_log(f"图床网络异常: {e}", "ERROR")
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTAUTO-Client/v42", "Cookie": self.config['cookie']}
...
class AddPresetDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent); self.setWindowTitle("添加 / 编辑预设方案"); self.resize(600, 700)
        layout = QVBoxLayout(self); form_layout = QFormLayout(); form_layout.setSpacing(14) 
        self.input_preset_name = QLineEdit(); form_layout.addRow("预设名称*:", QHBoxLayout(self).addWidget(self.input_preset_name) or self.input_preset_name)
        ...
        lbl_h.setContentsMargins(0, -5, 0, 5); form_layout.addRow("", lbl_h)
        
        self.input_intro = QTextEdit(); self.input_intro.setFixedHeight(120); form_layout.addRow("统一简介:", self.input_intro)
...
class ManagePresetsDialog(QDialog):
    ...
    def open_add_preset(self):
        dialog = AddPresetDialog(self); dialog.setStyleSheet(self.parent_win.styleSheet())
        if dialog.exec(): 
            d = dialog.get_data()
    ...
    def edit_preset(self, row):
        dialog = AddPresetDialog(self); dialog.setStyleSheet(self.parent_win.styleSheet()); dialog.set_data(self.parent_win.presets_data[row])
        if dialog.exec(): 
            nd = dialog.get_data()
    ...
    def refresh_table(self):
        ...
        for row, preset in enumerate(self.parent_win.presets_data):
            ...
            aw_copy = QWidget(); h_copy = QHBoxLayout(aw_copy); h_copy.setContentsMargins(2, 2, 2, 2)
            btn_copy = QPushButton("复制"); btn_copy.setStyleSheet("background-color: #34c759; color: white; padding: 2px; font-size: 11px; border:none; border-radius: 4px;"); btn_copy.clicked.connect(lambda checked, r=row: self.copy_preset(r))
            h_copy.addWidget(btn_copy); self.table.setCellWidget(row, 6, aw_copy)
            
            aw_act = QWidget(); h_act = QHBoxLayout(aw_act); h_act.setContentsMargins(2, 2, 2, 2); h_act.setSpacing(4)
            btn_edit = QPushButton("编辑"); btn_edit.setStyleSheet("background-color: #ff9500; color: white; padding: 2px; font-size: 11px; border:none; border-radius: 4px;"); btn_edit.clicked.connect(lambda checked, r=row: self.edit_preset(r))
            btn_del = QPushButton("删除"); btn_del.setStyleSheet("background-color: #ff3b30; color: white; padding: 2px; font-size: 11px; border:none; border-radius: 4px;"); btn_del.clicked.connect(lambda checked, r=row: self.delete_preset(r))
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTAUTO - 批量全自动发种工具 (极客优化版 v42)")
    ...
    def batch_add_folder(self):
        folder_path = QFileDialog.getExistingDirectory(self, "选择存放作品的文件夹", self.base_dir)
    ...
    def setup_settings_tab(self):
        ...
        pl.addWidget(self.create_hint_label("💡 保持源文件夹名称完全不变，适合已经规范命名好的资源。\n例：『主题』-摄影师-模特 -> 『主题』-摄影师-模特", "info"))
        
        pl.addWidget(self.rb_simple)
        pl.addWidget(self.create_hint_label("💡 保留原名称主体，自动扫描图片数量(P数)和年份，追加至末尾。\n例：『主题』-摄影师-模特 -> 『主题』-摄影师-模特-2023-36P-Moment", "info"))
        
        pl.addWidget(self.rb_full)
        pl.addWidget(self.create_hint_label("💡 严苛模式：根据源名称里的“-”强制拆解，缺失的模特/摄影师用预设补齐，严格重构成官方标准。\n例：『主题』-摄影师 -> 『主题』-预设模特-摄影师-2023-36P-Moment", "info"))
        fa.addRow("名称解析模式:", pl)
