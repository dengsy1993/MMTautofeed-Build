            ...
            if self.mode in ['publish', 'auto']:
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTautofeed-Client/v1.0.8", "Cookie": self.config['cookie']}
...
# =====================================================================
# 4. 弹窗类：添加/编辑预设 + 预设管理 (完美解决富文本污染导致字号失效)
# =====================================================================
class AddPresetDialog(QDialog):
    def __init__(self, parent=None, font_size=10):
        ...
        # 【完美修复】：彻底禁用富文本，所有粘贴进来的网页内容都会被强制洗净成纯文本
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.8 最终调优版)")
    ...
    # 【核心破案】：重写路径解析！不再粗暴砍掉左斜杠，完美兼容 Mac 的 /Users 根目录
    ...
    # 【效率进化】：引入 self.last_dir，所有弹窗自动记忆上次的路径！
