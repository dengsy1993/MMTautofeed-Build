...
# =====================================================================
# 2. 自定义 UI 控件: 终极三端完美多选下拉框 (修复了 Python/Win 假死)
# =====================================================================
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.view().viewport() and event.type() == QEvent.Type.MouseButtonRelease:
            ...
            if index.isValid():
                ...
                self.updateText()
            # 返回 True 彻底阻止事件继续传播，完美防止下拉菜单自动关闭！
            return True
            ...
            if self.mode in ['publish', 'auto']:
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTautofeed-Client/v1.0.10", "Cookie": self.config['cookie']}
                            ...
                            for dl_retry in range(3):
                                try:
                                    ...
                                    if dl_resp.status_code == 200 and len(dl_resp.content) > 100:
                                        ...
                                        if dl_retry == 2: self.emit_log("拉取官方种子失败，降级使用本地原始种子推送给 qB", "WARNING")
                                except Exception as e:
...
class AddPresetDialog(QDialog):
    def __init__(self, parent=None, font_size=10):
        ...
        self.input_intro = QTextEdit(); self.input_intro.setMinimumHeight(150)
        self.input_intro.setAcceptRichText(False) 
        
        self.input_intro.setStyleSheet(f"QTextEdit {{ font-size: {font_size}pt; }}")
        font = self.input_intro.font(); font.setPointSize(font_size)
        ...
        def update_font(v):
            ...
            f = self.input_intro.font(); f.setPointSize(v)
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.10 功德圆满版)")
