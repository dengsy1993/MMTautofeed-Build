    ...
    def run(self):
        ...
        for idx, task in enumerate(self.tasks):
            ...
            if self.mode in ['publish', 'auto']:
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTautofeed-Client/v1.0.12", "Cookie": self.config['cookie']}
                    ...
                    if resp.status_code in [200, 302] and match:
                        torrent_id = match.group(1); self.cell_update_signal.emit(row, 10, "✅ 发布成功", "#34c759"); self.emit_log(f"🎉【发布成功】成功获取新种子ID: {torrent_id}", "SUCCESS")
                        success_count += 1; set_p(90 if self.mode == 'auto' else 80)
                        if self.config['add_to_qb']:
                            dl_url = f"{self.config['pt_url']}download.php?id={torrent_id}"; self.emit_log(f"📥 [种子拉取] 请求带 Passkey 的官方种子...", "INFO")
                        ...
                        if 'login.php' in resp.url and not match: self.cell_update_signal.emit(row, 10, "❌ Cookie失效", "#ff3b30"); self.emit_log(f"🚨 被重定向至登录页，Cookie失效！", "ERROR")
            ...
            if idx < total_tasks - 1 and self.config.get('seed_delay', 3) > 0 and self.mode in ['publish', 'auto']:
                delay_sec = self.config['seed_delay']; self.emit_log(f"⏳ 触发发种限流保护，等待 {delay_sec} 秒后处理下一个任务...", "INFO")
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.12 锁定强化版)")
    ...
    def on_table_cell_changed(self, row, col):
        if col in [0, 5, 6]:
            ...
            try:
                ...
                f_name = self.table.item(row, 0).text() if self.table.item(row, 0) else ""; amount = self.table.item(row, 5).text() if self.table.item(row, 5) else ""; year = self.table.item(row, 6).text() if self.table.item(row, 6) else ""
                parse_mode = "simple"
                if self.rb_none.isChecked(): parse_mode = "none"
                elif self.rb_full.isChecked(): parse_mode = "full"
    ...
    def open_torrent_folder(self):
        path = self.get_abs_path(self.input_t_path.text()); os.makedirs(path, exist_ok=True)
    ...
    def test_pt_connection(self):
        try:
            ...
            if "login" in r.url: QMessageBox.warning(self, "提示", "能连通 PT 站，但被要求登录，请检查 Cookie。")
    ...
    def get_image_token(self):
        ...
        except Exception as e: QMessageBox.critical(self, "错误", f"请求异常: {e}")
