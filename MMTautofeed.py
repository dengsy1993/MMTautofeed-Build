    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.view().viewport() and event.type() == QEvent.Type.MouseButtonRelease:
            ...
            if index.isValid():
                ...
                item.setCheckState(Qt.CheckState.Unchecked if item.checkState() == Qt.CheckState.Checked else Qt.CheckState.Checked); self.updateText()
            return True
...
class BatchWorkerThread(QThread):
    ...
    def run(self):
        ...
        try: import torf
        except ImportError: self.emit_log("【环境报错】系统未安装 torf 库！请执行: pip install torf", "ERROR"); self.finished_signal.emit(0, total_tasks); return
        ...
        upload_form_url = f"{self.config['pt_url']}upload.php"; upload_submit_url = f"{self.config['pt_url']}takeupload.php"
        if self.mode in ['publish', 'auto'] and not self._tags_fetched:
            ...
            try:
                ...
                d_map = {name.strip(): val for val, name in matches if val.isdigit() and name}
                if d_map: self.tag_id_map.update(d_map); self.emit_log(f"✅ 标签字典同步成功({len(self.tag_id_map)})", "SUCCESS")
        ...
        for idx, task in enumerate(self.tasks):
            ...
            if self.mode in ['make', 'auto']:
                ...
                try:
                    ...
                    if use_zip:
                        set_p(15); zip_name = f"{std_name}.zip"; zip_path = os.path.join(seeding_dir, zip_name)
                        self.emit_log(f"📦 [文件打包] 制作 ZIP: {zip_name} ...", "INFO")
                    ...
                    self.cell_update_signal.emit(row, 8, "❌ 生成失败", "#ff3b30"); self.emit_log(f"制种出错: {e}", "ERROR"); set_p(100); continue
            ...
            if self.mode in ['publish', 'auto']:
                ...
                if task['thumb_path'] and os.path.exists(task['thumb_path']):
                    self.cell_update_signal.emit(row, 9, "⬆️ 上传图片...", "#ff9500"); self.emit_log(f"🖼️ [图床通信] 上传封面...", "INFO")
                    try:
                        ...
                        for retry in range(3):
                            try:
                                with open(task['thumb_path'], 'rb') as f:
                                    ...
                                    else: self.emit_log("⚠️ 警告: 未配置 Token！", "WARNING")
                                ...
                                else: self.emit_log(f"❌ 图床拒绝上传! 状态码: {img_res.status_code} | 拦截原因: {img_res.text[:150]}", "ERROR")
                                break 
                            except Exception as e: 
                ...
                self.emit_log("🔖 [表单组装] 组装分类与标签...", "INFO")
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTautofeed-Client/v1.0.11", "Cookie": self.config['cookie']}
                    ...
                    if resp.status_code in [200, 302] and match:
                        ...
                        self.cell_update_signal.emit(row, 9, "✅ 发布成功", "#34c759"); self.emit_log(f"🎉【发布成功】成功获取新种子ID: {torrent_id}", "SUCCESS")
                        ...
                        if self.config['add_to_qb']:
                            ...
                            self.emit_log(f"📥 [种子拉取] 请求带 Passkey 的官方种子...", "INFO")
                        ...
                        if 'login.php' in resp.url and not match: self.cell_update_signal.emit(row, 9, "❌ Cookie失效", "#ff3b30"); self.emit_log(f"🚨 被重定向至登录页，Cookie失效！", "ERROR")
                        else: self.cell_update_signal.emit(row, 9, f"⚠️ 频控或异常", "#ff3b30"); self.emit_log(f"🚨 页面未返回有效的种子 ID。", "ERROR")
                except Exception as e: self.cell_update_signal.emit(row, 9, "断网/超时", "#ff3b30"); self.emit_log(f"网络连接断开: {e}", "ERROR")
                set_p(100)
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.11 细节臻美版)")
        self.resize(1300, 880) 
        self.init_directories()
        icon_path_win = os.path.join(self.base_dir, 'app_icon.ico')
        icon_path_mac = os.path.join(self.base_dir, 'app_icon.icns')
        ...
        self.current_theme = "light"; self.hint_labels = []; self.presets_data = []; self.clean_keywords = []; self.clean_exts = []; self.preset_font_size = 10; self.last_dir = self.base_dir 
        self.load_presets()
        self.tabs = QTabWidget(); self.setCentralWidget(self.tabs); self.tab_batch = QWidget(); self.tab_settings = QWidget(); self.tab_log = QWidget()
        self.tabs.addTab(self.tab_batch, "⚡ 批量上传流水线"); self.tabs.addTab(self.tab_settings, "⚙️ 偏好设置"); self.tabs.addTab(self.tab_log, "🖥️ 运行日志监控")
        self.setup_log_tab(); self.setup_batch_upload_tab(); self.setup_settings_tab(); self.load_config()
    ...
    def log_msg(self, msg, level="INFO"):
        time_str = datetime.datetime.now().strftime("%H:%M:%S"); log_line = f"{time_str} | {level} | {msg}"
        if hasattr(self, 'log_view'): self.log_view.append(log_line); self.log_view.verticalScrollBar().setValue(self.log_view.verticalScrollBar().maximum())
        if hasattr(self, 'batch_log'): self.batch_log.append(log_line); self.batch_log.verticalScrollBar().setValue(self.batch_log.verticalScrollBar().maximum())
    ...
    def save_config(self, silent=False):
        ...
        config_data = {
            "theme": "dark" if self.combo_theme.currentIndex() == 1 else "light", "pt_url": self.input_pt_url.text(), "cookie": self.input_cookie.text(), "api_key": self.input_api_key.text(), "torrent_path": self.input_t_path.text(), "seeding_path": self.input_s_path.text(), "image_email": self.input_img_email.text(), "image_pwd": self.input_img_pwd.text(), "image_token": self.input_img_token.text(), "image_upload_api": self.input_img_upload_url.text(), "image_token_url": self.input_img_token_url.text(), "seed_delay": self.spin_delay.value(), "qb_url": self.input_qb_url.text(), "qb_user": self.input_qb_user.text(), "qb_pwd": self.input_qb_pwd.text(), "qb_auto_add": self.chk_qb_add.isChecked(), "anonymous": self.cb_batch_anon.isChecked(), "parse_mode": parse_mode, "clean_keywords": [self.list_kw.item(i).text() for i in range(self.list_kw.count())], "clean_exts": [self.list_ext.item(i).text() for i in range(self.list_ext.count())], "preset_font_size": self.preset_font_size
        }
        try:
    ...
    def load_config(self):
        ...
        if os.path.exists(config_file):
            try:
                ...
                self.input_pt_url.setText(config_data.get("pt_url") or "https://www.momentpt.top/"); self.input_cookie.setText(config_data.get("cookie", "")); self.input_api_key.setText(config_data.get("api_key", "")); self.input_t_path.setText(config_data.get("torrent_path") or "./torrents"); self.input_s_path.setText(config_data.get("seeding_path") or "./seeding")
                self.input_img_email.setText(config_data.get("image_email", "")); self.input_img_pwd.setText(config_data.get("image_pwd", "")); self.input_img_token.setText(config_data.get("image_token", "")); self.input_img_upload_url.setText(config_data.get("image_upload_api") or "https://img.momentpt.top/api/v1/upload"); self.input_img_token_url.setText(config_data.get("image_token_url") or "https://img.momentpt.top/api/v1/tokens")
                self.spin_delay.setValue(config_data.get("seed_delay", 3)); self.input_qb_url.setText(config_data.get("qb_url") or "http://127.0.0.1:8080"); self.input_qb_user.setText(config_data.get("qb_user") or "admin"); self.input_qb_pwd.setText(config_data.get("qb_pwd", ""))
                ...
                self.list_kw.clear(); self.list_kw.addItems(config_data.get("clean_keywords", [])); self.list_ext.clear(); self.list_ext.addItems(config_data.get("clean_exts", [])); self.preset_font_size = config_data.get("preset_font_size", 10)
            except Exception as e: self.log_msg(f"读取配置文件失败: {e}", "ERROR")
        else:
            self.apply_theme("light"); self.input_pt_url.setText("https://www.momentpt.top/"); self.input_img_upload_url.setText("https://img.momentpt.top/api/v1/upload"); self.input_img_token_url.setText("https://img.momentpt.top/api/v1/tokens"); self.input_t_path.setText("./torrents"); self.input_s_path.setText("./seeding"); self.spin_delay.setValue(3); self.input_qb_url.setText("http://127.0.0.1:8080"); self.input_qb_user.setText("admin"); self.chk_qb_add.setChecked(True); self.rb_simple.setChecked(True); self.preset_font_size = 10
    ...
    def preset_changed(self):
        ...
        if p_name == "请选择预设模板...": self.preset_info_label.setText("💡 请在左侧选择一个全局默认预设，作为后续新增资源的初始配置。")
    ...
    def add_single_folder_to_table(self, folder_path):
        ...
        self.table.setItem(row, 3, QTableWidgetItem(folder_path)); self.table.setItem(row, 4, QTableWidgetItem("未知")); self.table.setItem(row, 5, QTableWidgetItem("未知"))
        combo_cat = QComboBox(); combo_cat.addItems(SITE_CATEGORIES); self.table.setCellWidget(row, 6, combo_cat)
        combo_tags = CheckableComboBox(); combo_tags.set_items(GLOBAL_TAGS); self.table.setCellWidget(row, 7, combo_tags)
        self.table.setItem(row, 8, QTableWidgetItem("未制作")); self.table.setItem(row, 9, QTableWidgetItem("待处理"))
        btn_del = QPushButton("移除"); btn_del.setStyleSheet("background-color: #ff3b30; color: white; border: none; padding: 2px 4px; border-radius: 4px;"); btn_del.clicked.connect(self.remove_table_row); self.table.setCellWidget(row, 10, btn_del)
    ...
    def on_row_preset_changed(self, row):
        ...
        try:
            ...
            if cw := self.table.cellWidget(row, 6): cw.setCurrentText(preset.get("category", "写真"))
            if tw := self.table.cellWidget(row, 7): tw.set_items(GLOBAL_TAGS, preset.get("tags", []))
    ...
    def on_table_cell_changed(self, row, col):
        if col in [0, 4, 5]:
            ...
            try:
                ...
                f_name = self.table.item(row, 0).text() if self.table.item(row, 0) else ""; amount = self.table.item(row, 4).text() if self.table.item(row, 4) else ""; year = self.table.item(row, 5).text() if self.table.item(row, 5) else ""
                parse_mode = "simple"
                if self.rb_none.isChecked(): parse_mode = "none"
                elif self.rb_full.isChecked(): parse_mode = "full"
                
                if parse_mode == "none": std_name = f_name
                elif parse_mode == "simple": std_name = "-".join([x for x in [f"『{f_name}』" if f_name else "", preset.get("model", ""), preset.get("photographer", ""), year, amount, "Moment"] if x])
                elif parse_mode == "full":
                    raw_title = f_name; p_m = preset.get("model", ""); p_p = preset.get("photographer", "")
                    ...
                    if d_match: raw_title = raw_title.replace(d_match.group(1), "")
                        ...
                        if y_match: raw_title = raw_title.replace(y_match.group(1), "")
                    raw_title = re.sub(r'[-\s_]+', ' ', raw_title).strip(); std_name = "-".join([x for x in [f"『{raw_title}』" if raw_title else "", p_m, p_p, year, amount, "Moment"] if x])
    ...
    def batch_auto_match_thumbs(self):
        ...
        self.last_dir = thumb_dir; thumb_dir = self.get_abs_path(thumb_dir); mc = 0; self.batch_progress.setValue(0); self.log_msg(f"开始在文件夹 {thumb_dir} 中自动匹配封面...", "INFO")
        for row in range(self.table.rowCount()):
            ...
            if mp: self.table.item(row, 3).setData(Qt.ItemDataRole.UserRole, mp); self.table.setItem(row, 9, QTableWidgetItem("✅ 封面已匹配")); mc += 1
            else: self.table.setItem(row, 9, QTableWidgetItem("❌ 未找到封面"))
    ...
    def batch_manual_thumb(self):
        ...
        if fp: self.last_dir = os.path.dirname(fp); self.table.item(sel[0].topRow(), 3).setData(Qt.ItemDataRole.UserRole, fp); self.table.setItem(sel[0].topRow(), 9, QTableWidgetItem("✅ 封面已手动指定")); self.batch_progress.setValue(100)
    ...
    def batch_scan_folders(self):
        ...
        total_cleaned = 0; self.batch_progress.setValue(0)
        parse_mode = "simple"
        if self.rb_none.isChecked(): parse_mode = "none"
        elif self.rb_full.isChecked(): parse_mode = "full"
        self.log_msg(f"开始智能扫描提取 (采用提取模式: {parse_mode})", "INFO"); self.table.blockSignals(True)
        for row in range(self.table.rowCount()):
            f_path = self.table.item(row, 3).text(); f_name = os.path.basename(f_path); QApplication.processEvents()
            preset_name = self.table.cellWidget(row, 2).currentText() if self.table.cellWidget(row, 2) else ""; preset = next((p for p in self.presets_data if p["name"] == preset_name), {})
            if kws or exts:
                try:
            ...
            if parse_mode == "none": std_name = f_name
            elif parse_mode == "simple": std_name = "-".join([x for x in [f"『{f_name}』" if f_name else "", preset.get("model", ""), preset.get("photographer", ""), year, amount_str, "Moment"] if x])
            elif parse_mode == "full":
                raw_title = f_name; p_m = preset.get("model", ""); p_p = preset.get("photographer", "")
                if p_m and p_m in raw_title: raw_title = raw_title.replace(p_m, "")
                if p_p and p_p in raw_title: raw_title = raw_title.replace(p_p, "")
                ...
                if d_match: raw_title = raw_title.replace(d_match.group(1), "")
                    ...
                    y_match = re.search(r'\b(19\d{2}|20\d{2})\b', raw_title)
                    if y_match: raw_title = raw_title.replace(y_match.group(1), "")
                raw_title = re.sub(r'[-\s_]+', ' ', raw_title).strip(); std_name = "-".join([x for x in [f"『{raw_title}』" if raw_title else "", p_m, p_p, year, amount_str, "Moment"] if x])
            
            self.table.setItem(row, 1, QTableWidgetItem(std_name)); self.log_msg(f" -> 提取并组装名称: {std_name}")
            if cw := self.table.cellWidget(row, 6): cw.setCurrentText(preset.get("category", "写真"))
            if tw := self.table.cellWidget(row, 7): tw.set_items(GLOBAL_TAGS, preset.get("tags", []))
            self.table.setItem(row, 9, QTableWidgetItem("✅ 参数校验完毕")); self.batch_progress.setValue(int(((row + 1) / self.table.rowCount()) * 100))
    ...
    def update_table_cell(self, row, col, text, color_hex):
        item = QTableWidgetItem(text); item.setForeground(QColor(color_hex)); self.table.setItem(row, col, item)
    ...
    def start_worker(self, mode):
        ...
        for r in range(self.table.rowCount()):
            ...
            tasks.append({'row': r, 'std_name': self.table.item(r, 1).text().strip(), 'folder_path': self.table.item(r, 3).text().strip(), 'thumb_path': self.table.item(r, 3).data(Qt.ItemDataRole.UserRole), 'category': self.table.cellWidget(r, 6).currentText(), 'tag_names': self.table.cellWidget(r, 7).get_checked_items(), 'intro': row_preset.get('intro', '')})
    ...
    def _execute_auto_publish(self):
        if not self.batch_auto_match_thumbs(): self.log_msg("图库选择已取消，终止全自动流程。", "WARNING"); return
        self.start_worker('auto')

    def open_torrent_folder(self):
        path = self.get_abs_path(self.input_t_path.text()); os.makedirs(path, exist_ok=True); os.startfile(path) if sys.platform == 'win32' else subprocess.Popen(['open' if sys.platform == 'darwin' else 'xdg-open', path])
    ...
    def test_qb_connection(self):
        try:
            import qbittorrentapi
            qb = qbittorrentapi.Client(host=self.input_qb_url.text(), username=self.input_qb_user.text(), password=self.input_qb_pwd.text()); qb.auth_log_in(); QMessageBox.information(self, "成功", "已成功与本机的 qBittorrent 建立通信。")
    ...
    def get_image_token(self):
        ...
        try:
            ...
            if (token := data.get("token") or data.get("data", {}).get("token")): self.input_img_token.setText(token); QMessageBox.information(self, "成功", "已成功获取并填入 Token。")
    ...
    def setup_batch_upload_tab(self):
        ...
        group_mid = QGroupBox("第二步：装载文件夹与提取信息"); v_mid = QVBoxLayout(); h_toolbar = QHBoxLayout()
        ...
        b_clr = QPushButton("🗑 4.清空列表"); b_clr.setStyleSheet("background:#ff3b30; color:white; border:none;"); b_clr.clicked.connect(lambda: self.table.setRowCount(0))
        h_toolbar.addWidget(b_add); h_toolbar.addWidget(b_scn); h_toolbar.addWidget(b_rn); h_toolbar.addStretch(); h_toolbar.addWidget(b_clr); v_mid.addLayout(h_toolbar)

        self.table = QTableWidget(0, 11); self.table.verticalHeader().setDefaultSectionSize(34); self.table.setHorizontalHeaderLabels(["原始文件夹名", "最终种子名称", "应用预设", "物理路径", "P/V数", "年份", "分类", "附加标签", "种子", "状态", "操作"])
        head = self.table.horizontalHeader(); head.setSectionResizeMode(QHeaderView.ResizeMode.Interactive); self.table.setColumnWidth(0, 100); head.setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch); self.table.setColumnWidth(2, 110); self.table.setColumnWidth(3, 90); self.table.setColumnWidth(4, 70); self.table.setColumnWidth(5, 60); self.table.setColumnWidth(6, 75); self.table.setColumnWidth(7, 120); self.table.setColumnWidth(8, 75); self.table.setColumnWidth(9, 80); self.table.setColumnWidth(10, 50)
        ...
        b_auto_img = QPushButton("① 自动寻找封面"); b_auto_img.clicked.connect(self.batch_auto_match_thumbs)
        b_man_img = QPushButton("① 手动指定封面"); b_man_img.clicked.connect(self.batch_manual_thumb)
        self.btn_make = QPushButton("② 后台打包制种"); self.btn_make.clicked.connect(lambda: self.start_worker('make'))
        self.btn_pub = QPushButton("③ 推送到 PT 站"); self.btn_pub.clicked.connect(lambda: self.start_worker('publish'))
        b5 = QPushButton("📁 打开种子目录"); b5.clicked.connect(self.open_torrent_folder)
        h_exec.addWidget(QLabel("分步执行:")); h_exec.addWidget(b_auto_img); h_exec.addWidget(b_man_img); h_exec.addWidget(self.btn_make); h_exec.addWidget(self.btn_pub); h_exec.addWidget(b5); h_exec.addStretch(); v_bot.addLayout(h_exec)
        
        h_main_btn = QHBoxLayout(); self.btn_auto = QPushButton("🚀 一键全自动上传 (推荐)"); self.btn_auto.setStyleSheet("background: #007aff; color: white; font-size: 14px; padding: 10px 24px; border: none;"); self.btn_auto.clicked.connect(self.run_auto_publish)
        ...
        lbl_desc = self.create_hint_label("💡 流程说明：1.添加文件夹 ➔ 2.扫描提取参数 ➔ 3.启动自动化上传 (包含自动找图/打ZIP/上图床/发种/自动做种)", "primary"); v_bot.addWidget(lbl_desc)
    ...
    def get_hline(self): line = QFrame(); line.setFrameShape(QFrame.Shape.HLine); line.setStyleSheet("background-color: rgba(150, 150, 150, 100); max-height: 1px; margin: 8px 0;"); return line
