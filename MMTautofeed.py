            ...
            if self.mode in ['publish', 'auto']:
                ...
                try:
                    ...
                    with open(torrent_path, "rb") as file_stream:
                        ...
                        headers = {"User-Agent": "MMTautofeed-Client/v1.0.4", "Cookie": self.config['cookie']}
...
# =====================================================================
# 4. 弹窗类：添加/编辑预设 + 预设管理 (完美解决 Mac 截断与字号调节)
# =====================================================================
class AddPresetDialog(QDialog):
    def __init__(self, parent=None, font_size=10):
        ...
        # 【全平台字体自适应：同时注入 CSS 和系统 QFont】
        self.input_intro.setStyleSheet(f"QTextEdit {{ font-size: {font_size}pt; }}")
        font = self.input_intro.font(); font.setPointSize(font_size); self.input_intro.setFont(font)
        ...
        def update_font(v):
            ...
            f = self.input_intro.font(); f.setPointSize(v); self.input_intro.setFont(f)
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.4)")
    ...
    def batch_add_folder(self):
        # 【Mac 多选兼容】：这里强制使用 Qt 内置对话框，因为原生 Finder 在代码层面不支持多选文件夹
        dialog = QFileDialog(self, "选择存放作品的多个文件夹 (Win按Ctrl / Mac按Command 多选)", self.base_dir)
    ...
    # 【Mac 原生恢复】：因为只需要单选，去掉了 DontUseNativeDialog，全平台均使用原生文件选择器！
    ...
    # 【Mac 完美防崩溃】：使用定时器分离线程，切断弹窗与 Finder 对话框的死锁关联
    def run_auto_publish(self):
        if QMessageBox.question(self, "一键全自动", "将依次执行：自动找图 ➔ 打包制种 ➔ 推送图床 ➔ 表单发布 ➔ 添加做种。是否继续？", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            QTimer.singleShot(200, self._execute_auto_publish)
