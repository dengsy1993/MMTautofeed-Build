...
# =====================================================================
# 2. 自定义 UI 控件: 终极三端完美多选下拉框 (修复了点击文字的问题)
# =====================================================================
class CheckableComboBox(QComboBox):
    ...
    def get_checked_items(self): 
        return [self.model().item(i).text() for i in range(self.count()) if self.model().item(i).checkState() == Qt.CheckState.Checked]
...
# =====================================================================
# 3. 核心发种大动脉：后台多线程流水线引擎 (带网络重试与防超时容错)
# =====================================================================
class BatchWorkerThread(QThread):
    log_signal = pyqtSignal(str, str)
    progress_signal = pyqtSignal(int)
    cell_update_signal = pyqtSignal(int, int, str, str)
    ...
    def run(self):
        ...
        try: 
            ...
            self.emit_log("【环境报错】系统未安装 torf 库！请在终端执行: pip install torf", "ERROR")
            self.finished_signal.emit(0, total_tasks)
            return

        torrent_dir = self.config['torrent_dir']
        seeding_dir = self.config['seeding_dir'] 
        ...
        os.makedirs(local_torrent_dir, exist_ok=True)
        os.makedirs(pt_torrent_dir, exist_ok=True)
        
        announce_url = self.config['pt_url'] + "announce.php"
        use_zip = self.config['use_zip']
        ...
        if self.mode in ['publish', 'auto'] and not self._tags_fetched:
            ...
            try:
                ...
                self.emit_log(f"标签库同步网络异常，启用内置密码本: {e}", "WARNING")
            self._tags_fetched = True

        for idx, task in enumerate(self.tasks):
            if not self.is_running: 
                self.emit_log("🛑 任务已被中止！", "WARNING")
                break
                
            row, std_name, folder_path = task['row'], task['std_name'], task['folder_path']
            base_prog = (idx / total_tasks) * 100
            task_weight = 100 / total_tasks
            
            def set_p(percent): 
                self.progress_signal.emit(int(base_prog + (percent / 100.0) * task_weight))
            ...
            if self.mode in ['make', 'auto']:
                if not os.path.exists(folder_path): 
                    self.cell_update_signal.emit(row, 8, "❌ 路径失效", "#ff3b30")
                    set_p(100)
                    continue
                ...
                try:
                    ...
                    if use_zip:
                        set_p(15)
                        zip_name = f"{std_name}.zip"
                        zip_path = os.path.join(seeding_dir, zip_name)
                    ...
                    t = torf.Torrent(path=target_path, trackers=[announce_url], private=True)
                    t.generate()
                    ...
                    self.cell_update_signal.emit(row, 8, "❌ 生成失败", "#ff3b30")
                    self.emit_log(f"制种出错 ({std_name}): {e}", "ERROR")
                    set_p(100)
                    continue
            ...
            if self.mode in ['publish', 'auto']:
                ...
                if task['thumb_path'] and os.path.exists(task['thumb_path']):
                    self.cell_update_signal.emit(row, 9, "⬆️ 上传图片...", "#ff9500")
                    self.emit_log(f"🖼️ [图床通信] 开始上传展示封面...", "INFO")
                    try:
                        ...
                        for retry in range(3):
                            try:
                                ...
                                self.emit_log(f"⚠️ 图床网络波动({e})，发起重试 {retry+1}/3", "WARNING")
                                self.msleep(2000)
                                if retry == 2: raise
                    except Exception as e: 
                        self.emit_log(f"❌ 图床网络通讯彻底失败，详细错误: {e}", "ERROR")
                ...
                if self.config['test_mode']:
                    ...
                    self.cell_update_signal.emit(row, 9, "✅ 模拟完毕", "#34c759")
                    success_count += 1
                    set_p(100)
                    continue
                ...
                if not os.path.exists(torrent_path): 
                    torrent_path = os.path.join(local_torrent_dir, f"{std_name}.zip.torrent")
                if not os.path.exists(torrent_path): 
                    self.cell_update_signal.emit(row, 9, "❌ 未制作种子", "#ff3b30")
                    set_p(100)
                    continue
                
                self.cell_update_signal.emit(row, 9, "🚀 推送中...", "#ff9500")
                set_p(80 if self.mode == 'auto' else 60)
                ...
                try:
                    ...
                    if resp.status_code in [200, 302] and match:
                        ...
                        self.cell_update_signal.emit(row, 9, "✅ 发布成功", "#34c759")
                        self.emit_log(f"🎉【发布成功】成功获取站点新分配的种子ID: {torrent_id}", "SUCCESS")
                        success_count += 1
                        set_p(90 if self.mode == 'auto' else 80)
                            ...
                            for dl_retry in range(3):
                                try:
                                    ...
                                    if dl_resp.status_code == 200 and len(dl_resp.content) > 100:
                                        ...
                                        with open(final_torrent_path, "wb") as tf: 
                                    ...
                                    self.emit_log(f"⚠️ 官方种子拉取超时({e})，发起重试 {dl_retry+1}/3", "WARNING")
                                    self.msleep(2000)
                        ...
                        if 'login.php' in resp.url and not match:
                            self.cell_update_signal.emit(row, 9, "❌ Cookie失效", "#ff3b30")
                            self.emit_log(f"🚨 被重定向至登录页，Cookie失效或权限不足！", "ERROR")
                        else:
                            self.cell_update_signal.emit(row, 9, f"⚠️ 频控或异常", "#ff3b30")
                            self.emit_log(f"🚨 页面未返回有效的种子 ID。可能必填项为空或触发了站点防刷机制。", "ERROR")
                            
                except Exception as e:
                    self.cell_update_signal.emit(row, 9, "断网/超时", "#ff3b30")
                    self.emit_log(f"网络连接断开: {e}", "ERROR")
                set_p(100)
    ...
    def push_to_qb(self, torrent_path, save_dir):
        try:
            ...
            self.emit_log(f"⚠️ qB 客户端无法连接: {e}", "WARNING")
# =====================================================================
# 4. 弹窗类：添加/编辑预设 + 预设管理 (防富文本污染)
# =====================================================================
class AddPresetDialog(QDialog):
    def __init__(self, parent=None, font_size=10):
        super().__init__(parent)
        self.setWindowTitle("添加 / 编辑预设方案")
        self.resize(980, 700)
        ...
        form_layout.addRow("预设名称*:", self.input_preset_name)
        self.input_photographer = QLineEdit()
        form_layout.addRow("摄影师:", self.input_photographer)
        self.input_model = QLineEdit()
        form_layout.addRow("主角/模特:", self.input_model)
        
        lbl_h = QLabel("💡 提示：至少填写【摄影师】或【主角/模特】之一才能正确生成名字")
        lbl_h.setStyleSheet("color: #ff3b30; font-size: 12px;")
        lbl_h.setContentsMargins(0, 0, 0, 0)
        form_layout.addRow("", lbl_h)
        
        self.input_intro = QTextEdit()
        self.input_intro.setMinimumHeight(150)
        self.input_intro.setAcceptRichText(False) 
        
        self.input_intro.setStyleSheet(f"QTextEdit {{ font-size: {font_size}pt; }}")
        font = self.input_intro.font()
        font.setPointSize(font_size)
        self.input_intro.setFont(font)
        self.input_intro.document().setDefaultFont(font) 
        
        w_intro = QWidget()
        h_intro = QHBoxLayout(w_intro)
        h_intro.setContentsMargins(0, 0, 0, 0)
        h_intro.addWidget(QLabel("统一简介:"))
        h_intro.addStretch()
        h_intro.addWidget(QLabel("字号调节:"))
        
        self.spin_intro_font = QSpinBox()
        self.spin_intro_font.setRange(9, 24)
        self.spin_intro_font.setValue(font_size)
        
        def update_font(v):
            self.input_intro.setStyleSheet(f"QTextEdit {{ font-size: {v}pt; }}")
            f = self.input_intro.font()
            f.setPointSize(v)
            self.input_intro.setFont(f)
            self.input_intro.document().setDefaultFont(f)
            
        self.spin_intro_font.valueChanged.connect(update_font)
        ...
        self.combo_category.addItems(SITE_CATEGORIES)
        form_layout.addRow("默认分类:", self.combo_category)
        layout.addLayout(form_layout)
        
        group_tags = QGroupBox("默认附加标签 (已隐去底层强制绑定的 官方、禁转)")
        ...
        for i, tag in enumerate(GLOBAL_TAGS):
            ...
            tags_layout.addWidget(cb, i // 5, i % 5)
        ...
        btn_save = QPushButton("💾 保存设定")
        btn_save.setStyleSheet("background-color: #34c759; color: white;")
    ...
    def set_data(self, data):
        self.input_preset_name.setText(data.get("name", ""))
        self.input_photographer.setText(data.get("photographer", ""))
        self.input_model.setText(data.get("model", ""))
        self.input_intro.setText(data.get("intro", ""))
        self.combo_category.setCurrentText(data.get("category", "请选择分类..."))
        for tag, cb in self.checkboxes.items(): 
            cb.setChecked(tag in data.get("tags", []))

    def get_data(self):
        return {
            "name": self.input_preset_name.text().strip(), 
            "photographer": self.input_photographer.text().strip(), 
            "model": self.input_model.text().strip(), 
            "intro": self.input_intro.toPlainText().strip(), 
            "category": self.combo_category.currentText(), 
            "tags": [t for t, c in self.checkboxes.items() if c.isChecked()]
        }

class ManagePresetsDialog(QDialog):
    def __init__(self, parent_main_window):
        super().__init__(parent_main_window)
        self.parent_win = parent_main_window
        self.setWindowTitle("预设配置管理")
        self.resize(980, 600)
        layout = QVBoxLayout(self)
        h_top = QHBoxLayout()
        
        btn_add = QPushButton("十 新增一条预设")
        btn_add.setStyleSheet("background-color: #007aff; color: white; border: none; padding: 6px;")
        btn_add.clicked.connect(self.open_add_preset)
        ...
        self.table.setHorizontalHeaderLabels(["模板名称", "摄影师", "模特", "简介", "分类", "标签", "复制", "操作"])
    ...
    def open_add_preset(self):
        dialog = AddPresetDialog(self, font_size=self.parent_win.preset_font_size)
        dialog.setStyleSheet(self.parent_win.styleSheet())
        if dialog.exec(): 
            d = dialog.get_data()
            if not d["name"]: return QMessageBox.warning(self, "错误", "名称不能为空！")
            self.parent_win.presets_data.append(d)
            self.parent_win.save_presets()
            self.refresh_table()
            self.parent_win.refresh_main_preset_combo()
            self.parent_win.preset_font_size = dialog.get_font_size()
            self.parent_win.save_config(silent=True)

    def edit_preset(self, row):
        dialog = AddPresetDialog(self, font_size=self.parent_win.preset_font_size)
        dialog.setStyleSheet(self.parent_win.styleSheet())
        dialog.set_data(self.parent_win.presets_data[row])
        if dialog.exec(): 
            nd = dialog.get_data()
            if not nd["name"]: return
            self.parent_win.presets_data[row] = nd
            self.parent_win.save_presets()
            self.refresh_table()
            self.parent_win.refresh_main_preset_combo()
            self.parent_win.preset_font_size = dialog.get_font_size()
    ...
    def refresh_table(self):
        ...
        for row, preset in enumerate(self.parent_win.presets_data):
            ...
            self.table.setItem(row, 0, QTableWidgetItem(preset.get("name", "")))
            self.table.setItem(row, 1, QTableWidgetItem(preset.get("photographer", "")))
            self.table.setItem(row, 2, QTableWidgetItem(preset.get("model", "")))
            self.table.setItem(row, 3, QTableWidgetItem(preset.get("intro", "")))
            self.table.setItem(row, 4, QTableWidgetItem(preset.get("category", "")))
            self.table.setItem(row, 5, QTableWidgetItem(", ".join(preset.get("tags", []))))
            
            aw_copy = QWidget()
            h_copy = QHBoxLayout(aw_copy)
            h_copy.setContentsMargins(6, 4, 6, 4)
            btn_copy = QPushButton("复制")
            btn_copy.setStyleSheet("background-color: #34c759; color: white; padding: 4px; font-size: 11px; border:none; border-radius: 4px;")
            btn_copy.clicked.connect(lambda checked, r=row: self.copy_preset(r))
            h_copy.addWidget(btn_copy)
            self.table.setCellWidget(row, 6, aw_copy)
            
            aw_act = QWidget()
            h_act = QHBoxLayout(aw_act)
            h_act.setContentsMargins(4, 4, 4, 4)
            h_act.setSpacing(4)
            btn_edit = QPushButton("编辑")
            btn_edit.setStyleSheet("background-color: #ff9500; color: white; padding: 4px; font-size: 11px; border:none; border-radius: 4px;")
            btn_edit.clicked.connect(lambda checked, r=row: self.edit_preset(r))
            
            btn_del = QPushButton("删除")
            btn_del.setStyleSheet("background-color: #ff3b30; color: white; padding: 4px; font-size: 11px; border:none; border-radius: 4px;")
            btn_del.clicked.connect(lambda checked, r=row: self.delete_preset(r))
            
            h_act.addWidget(btn_edit)
            h_act.addWidget(btn_del)
            self.table.setCellWidget(row, 7, aw_act)
    ...
    def delete_preset(self, row):
        if QMessageBox.question(self, '确认删除', "确定永久删除此条预设？", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            del self.parent_win.presets_data[row]
            self.parent_win.save_presets()
            self.refresh_table()
            self.parent_win.refresh_main_preset_combo()
...
class TagsSelectDialog(QDialog):
    def __init__(self, current_tags, parent=None):
        super().__init__(parent)
        self.setWindowTitle("选择发种标签")
        self.resize(400, 250)
        layout = QVBoxLayout(self)
        
        lbl = QLabel("💡 提示：系统将在发包时自动在后台附加【官方】与【禁转】标签，此处无需勾选。")
        lbl.setStyleSheet("color: #e65100; font-size: 12px; margin-bottom: 10px;")
        ...
        for i, t in enumerate(SITE_TAGS):
            cb = QCheckBox(t)
            cb.setChecked(t in current_tags)
            self.cbs[t] = cb
        ...
        btn_ok.setStyleSheet("background-color: #007aff; color: white;")
        btn_ok.clicked.connect(self.accept)
        layout.addWidget(btn_ok, alignment=Qt.AlignmentFlag.AlignRight)

    def get_selected(self): 
        return [t for t, cb in self.cbs.items() if cb.isChecked()]
...
class PTUploaderFullGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MMTautofeed批量发种工具 (v1.0.11)")
    ...
    def load_config(self):
        ...
        if os.path.exists(config_file):
            try:
                ...
                self.list_kw.clear()
                self.list_kw.addItems(config_data.get("clean_keywords", []))
                self.list_ext.clear()
                self.list_ext.addItems(config_data.get("clean_exts", []))
    ...
    def batch_add_folder(self):
        dialog = QFileDialog(self, "选择存放作品的多个文件夹 (Win按Ctrl / Mac按Command 多选)", self.last_dir)
    ...
    def batch_manual_thumb(self):
        ...
        if fp: 
            self.last_dir = os.path.dirname(fp)
    ...
    def run_auto_publish(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if sys.platform == 'darwin':
                QTimer.singleShot(400, self._execute_auto_publish)
            else:
                self._execute_auto_publish()
